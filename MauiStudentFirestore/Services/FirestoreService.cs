...
using MauiStudentFirestore.Models;

namespace MauiStudentFirestore.Services;
...
public class FirestoreService
{
     ...
     private async Task SetupFireStore()
    {
        if (db == null)
        {
            var stream = await FileSystem.OpenAppPackageFileAsync("dx212-2024-9e1fb-firebase-adminsdk-8gxbb-abd04e30ac.json");
            var reader = new StreamReader(stream);
            var contents = reader.ReadToEnd();
            db = new FirestoreDbBuilder
            {
                ProjectId = "dx212-2024-9e1fb",
